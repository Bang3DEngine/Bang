...
#define PX_ANDROID 1
    ...
    defined(__EMSCRIPTEN__)  // note: __ANDROID__ implies __linux__
#define PX_LINUX 1
...
*/
    ...
    defined(_M_X64)  // ps4 compiler defines _M_X64 without value
#define PX_X64 1
...
*/
    ...
    defined(_M_X64) || (defined(__EMSCRIPTEN__) && defined(__SSE2__))
#define PX_SSE2 1
#endif
...
#define PX_UNIX_FAMILY \
    (PX_LINUX_FAMILY || PX_APPLE_FAMILY)  // shortcut for unix/posix platforms
...
*/
#if defined(_LIBCPP_VERSION) || PX_WIN64 || PX_WIN32 || PX_PS4 || \
    PX_XBOXONE || PX_EMSCRIPTEN
#define PX_LIBCPP 1
#else
#define PX_LIBCPP 0
#endif
...
PX_FOUNDATION_DLL=0 - for windows configurations where the PX_FOUNDATION_API is
linked through standard static linking no definition       - this will allow
DLLs and libraries to use the exported API from PhysXCommon
...
#elif PX_LINUX  // Workaround; Fedora Core 3 do not agree with force inline and
                // PxcPool
...
PX_ALIGN_PREFIX and PX_ALIGN_SUFFIX can be used for type alignment instead of
aligning individual variables as follows: PX_ALIGN_PREFIX(16) struct A {
...
} PX_ALIGN_SUFFIX(16);
...
/**
Deprecated macro
- To deprecate a function: Place PX_DEPRECATED at the start of the function
header (leftmost word).
- To deprecate a 'typedef', a 'struct' or a 'class': Place PX_DEPRECATED
directly after the keywords ('typdef', 'struct', 'class').
...
#define PX_COMPILE_TIME_ASSERT(exp)                     \
    typedef char PX_CONCAT(PxCompileTimeAssert_Dummy,   \
                           __COUNTER__)[(exp) ? 1 : -1] \
        __attribute__((unused))
#else
#define PX_COMPILE_TIME_ASSERT(exp) \
    typedef char PxCompileTimeAssert_Dummy[(exp) ? 1 : -1]
#endif
...
#define PX_OFFSETOF_BASE \
    0x100  // casting the null ptr takes a special-case code path, which we
           // don't want
#define PX_OFFSET_OF_RT(Class, Member)                           \
    (reinterpret_cast<size_t>(                                   \
         &reinterpret_cast<Class *>(PX_OFFSETOF_BASE)->Member) - \
     size_t(PX_OFFSETOF_BASE))
...
// assert works on win32/win64, but may need further specialization on other
// platforms. Some GCC compilers need the compiler flag -malign-double to be
// set. Apparently the apple-clang-llvm compiler doesn't support malign-double.
...
#endif
// clang (as of version 3.9) cannot align doubles on 8 byte boundary  when
// compiling for Intel 32 bit target
...
#define PX_WEAK_SYMBOL \
    __attribute__((weak))  // this is to support SIMD constant merging in
                           // template specialization
#else
#define PX_WEAK_SYMBOL
#endif

// Macro for avoiding default assignment and copy, because doing this by
// inheritance can increase class size on some platforms.

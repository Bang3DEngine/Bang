...
#define D_MAX_BLOCKS_IN_MCU 10 /* decompressor's limit on blocks per MCU */
    ...
    int DCT_scaled_size;
         ...
         * similarly for height.  For decompression, IDCT scaling is included,
         * so downsampled_width = ceil(image_width * Hi/Hmax *
         * DCT_[h_]scaled_size/DCTSIZE)
         */
...
#define JCS_ALPHA_EXTENSIONS 1
    ...
    typedef enum
    {
        ...
        /* When out_color_space it set to JCS_EXT_RGBX, JCS_EXT_BGRX,
           ...
           undefined, and in order to ensure the best performance, libjpeg-turbo
           can set that byte to whatever value it wishes.  Use the following
           colorspace constants to ensure that the X byte is set to 0xFF, so
           that it can be interpreted as an opaque alpha channel. */
...
#define jpeg_common_fields                                                \
     ...
     * jpeg_common_struct, only of jpeg_compress_struct and
    ...
    struct jpeg_compress_struct
    {
         ...
         * burnt when new parameters are added.  Also note that there are
         * several helper routines to simplify changing parameters.
         */
         ...
         * You can also use jpeg_calc_jpeg_dimensions() to determine these
         * values in advance of calling jpeg_start_compress().
         */
#endif
        ...
        UINT8 arith_dc_U[NUM_ARITH_TBLS]; /* U values for DC arith-coding tables
                                           */
        UINT8 arith_ac_K[NUM_ARITH_TBLS]; /* Kx values for AC arith-coding
                                             tables */
        ...
        /* The coefficient controller receives data in units of MCU rows as
         * defined for fully interleaved scans (whether the JPEG file is
         * interleaved or not). There are v_samp_factor * DCTSIZE sample rows of
         * each component in an "iMCU" (interleaved MCU) row.
         */
    ...
    struct jpeg_decompress_struct
    {
        ...
        unsigned int restart_interval; /* MCUs per restart interval, or 0 for no
                                          restart */
    ...
    int min_DCT_scaled_size; /* smallest DCT_scaled_size of any component */
         ...
         * v_samp_factor*DCT_[v_]scaled_size sample rows of a component per iMCU
         * row.
         */
...
#define JMSG_STR_PARM_MAX 80
        ...
        /* Second table can be added by application (see cjpeg/djpeg for
         * example). It contains strings numbered
     ...
     * at once.  This is faster and more convenient than remembering exactly
     * what to free, especially where malloc()/free() are not too speedy. NB:
     * alloc routines never return NULL.  They exit to error_exit if not
     * successful.
     */
...
#define JPOOL_NUMPOOLS 2
    ...
    struct jpeg_memory_mgr
    {
        ...
        void *(*alloc_small)(j_common_ptr cinfo,
                             int pool_id,
                             size_t sizeofobject);
        void *(*alloc_large)(j_common_ptr cinfo,
                             int pool_id,
                             size_t sizeofobject);
...
#define JPEG_HEADER_TABLES_ONLY 2 /* Found valid table-specs-only datastream \
    ...
    EXTERN(void)
    jpeg_crop_scanline(j_decompress_ptr cinfo,
                       JDIMENSION *xoffset,
                       JDIMENSION *width);
...
/* Precalculate output dimensions for current decompression parameters. */
    ...
    /* These marker codes are exported since applications and data source
     * modules are likely to want to use them.
     */
...
#define JPEG_COM 0xFE  /* COM marker code */
    ...
    /* If we have a brain-damaged compiler that emits warnings (or worse,
     * errors) for structure definitions that are never filled in, keep it quiet
     * by supplying dummy definitions for the various substructures.
     */
     ...
     * file. The internal structure declarations are read only when that is
     * true. Applications using the library should not include jpegint.h, but
     * may wish to include jerror.h.
     */

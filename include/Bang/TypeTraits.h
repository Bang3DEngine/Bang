...
#define TT_SCALAR(T) typename std::enable_if<T_SCALAR(T), T>::type
#define TT_INTEGRAL(T) typename std::enable_if<T_INTEGRAL(T), T>::type
#define TT_FLOATING(T) typename std::enable_if<T_FLOATING(T), T>::type
#define TT_ENUM(T) typename std::enable_if<T_ENUM(T), T>::type
#define TT_POINTER(T) typename std::enable_if<T_POINTER(T), T>::type
#define TT_NOT_POINTER(T) typename std::enable_if<!T_POINTER(T), T>::type
...
struct IsExplicitlyConvertible
    : public std::__and_<std::is_constructible<SourceT, DestT>,
                         std::__not_<std::is_convertible<DestT, SourceT>>>::type
{
};

template <typename SourceT, typename DestT>
struct IsStaticCasteable
    : public std::__or_<std::is_enum<SourceT>,
                        ...
                        IsExplicitlyConvertible<DestT, SourceT>>::type
{
};

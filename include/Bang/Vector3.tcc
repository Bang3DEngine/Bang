...
Vector3G<T>::Vector3G() 
{ 
    for (int i = 0; i < 3; ++i) { At(i) = SCAST<T>(0); } 
} 

template<class T> 
Vector3G<T>::Vector3G(const T& a) 
{ 
    for (int i = 0; i < 3; ++i) { At(i) = SCAST<T>(a); } 
} 
...
{ 
    for (int i = 0; i < 3; ++i) { At(i) = SCAST<T>(v[i]); } 
} 
...
{ 
    T res = SCAST<T>(0); 
    for (int i = 0; i < 3; ++i) { res += At(i) * At(i); } 
    return res; 
} 
...
{ 
    if (*this == Vector3G<T>::Zero) { return Vector3G<T>::Zero; } 
...
{ 
    Vector3G<T> res(*this); 
    for (int i = 0; i < 3; ++i) { res[i] = Math::RadToDeg( res[i] ); } 
    return res; 
} 
...
{ 
    Vector3G<T> res(*this); 
    for (int i = 0; i < 3; ++i) { res[i] = Math::DegToRad( res[i] ); } 
    return res; 
} 
...
{ 
    Vector3G res(*this); 
    for (int i = 0; i < 3; ++i) { res[i] = Math::Abs( res[i] ); } 
    return res; 
} 

template<class T> 
T* Vector3G<T>::Data() { return &At(0); } 
template<class T> 
const T* Vector3G<T>::Data() const { return &At(0); } 
...
{ 
    Vector3G res; 
    for (int i = 0; i < 3; ++i) { res[i] = Math::Max(v1[i], v2[i]); } 
    return res; 
} 
...
{ 
    Vector3G res; 
    for (int i = 0; i < 3; ++i) { res[i] = Math::Min(v1[i], v2[i]); } 
    return res; 
} 
...
{ 
    Vector3G res; 
    for (int i = 0; i < 3; ++i) { res[i] = Math::Floor(v1[i]); } 
    return res; 
} 
...
{ 
    Vector3G res; 
    for (int i = 0; i < 3; ++i) { res[i] = Math::Ceil(v1[i]); } 
    return res; 
} 
...
{ 
    Vector3G res; 
    for (int i = 0; i < 3; ++i) { res[i] = Math::Round(v1[i]); } 
    return res; 
} 
...
{ 
    Vector3G res = v; 
    for (int i = 0; i < 3; ++i) { res[i] = Math::Clamp(res[i], min[i], max[i]); } 
    return res; 
} 
...
template<class T>
Vector3G<T> Vector3G<T>::ProjectedOnPlane(
                         const Vector3G<T> &planeNormal, 
...
template<class T>
Vector2G<T> Vector3G<T>::xy() const { return Vector2G<T>(x, y); } 

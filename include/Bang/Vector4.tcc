...
Vector4G<T>::Vector4G() 
{ 
    for (int i = 0; i < 4; ++i) { At(i) = SCAST<T>(0); } 
} 

template<class T> 
Vector4G<T>::Vector4G(const T& a) 
{ 
    for (int i = 0; i < 4; ++i) { At(i) = SCAST<T>(a); } 
} 
...
{ 
    for (int i = 0; i < 4; ++i) { At(i) = SCAST<T>(v[i]); } 
}
...
Vector4G<T>::Vector4G(const Vector2G<OtherT1> &v, const OtherT2 &_z, const OtherT3 &_w)
            : x(SCAST<T>(v.x)),
              y(SCAST<T>(v.y)),
              z(SCAST<T>(_z)),
              w(SCAST<T>(_w)) {}
...
template<class T>
template <class OtherT1, class OtherT2, class OtherT3>
Vector4G<T>::Vector4G(const OtherT1 &_x, const Vector2G<OtherT2> &v, const OtherT3 &_w)
            : x(SCAST<T>(_x)),
              y(SCAST<T>(v.y)),
              z(SCAST<T>(v.z)),
              w(SCAST<T>(_w)) {}
...
template<class T>
template <class OtherT1, class OtherT2, class OtherT3>
Vector4G<T>::Vector4G(const OtherT1 &_x, const OtherT2 &_y, const Vector2G<OtherT3> &v)
            : x(SCAST<T>(_x)),
              y(SCAST<T>(_y)),
              z(SCAST<T>(v.z)),
              w(SCAST<T>(v.w)) {}
...
{ 
    T res = SCAST<T>(0); 
    for (int i = 0; i < 4; ++i) { res += At(i) * At(i); } 
    return res; 
} 
...
{ 
    if (*this == Vector4G<T>::Zero) { return Vector4G<T>::Zero; } 
...
{ 
    Vector4G<T> res(*this); 
    for (int i = 0; i < 4; ++i) { res[i] = Math::RadToDeg( res[i] ); } 
    return res; 
} 
...
{ 
    Vector4G<T> res(*this); 
    for (int i = 0; i < 4; ++i) { res[i] = Math::DegToRad( res[i] ); } 
    return res; 
} 
...
{ 
    Vector4G res(*this); 
    for (int i = 0; i < 4; ++i) { res[i] = Math::Abs( res[i] ); } 
    return res; 
} 

template<class T> 
T* Vector4G<T>::Data() { return &At(0); } 
template<class T> 
const T* Vector4G<T>::Data() const { return &At(0); } 
...
{ 
    T res = SCAST<T>(0); 
    for (int i = 0; i < 4; ++i) { res += v1[i] * v2[i]; } 
    return res; 
} 
...
{ 
    Vector4G res; 
    for (int i = 0; i < 4; ++i) { res[i] = Math::Max(v1[i], v2[i]); } 
    return res; 
} 
...
{ 
    Vector4G res; 
    for (int i = 0; i < 4; ++i) { res[i] = Math::Min(v1[i], v2[i]); } 
    return res; 
} 
...
{ 
    Vector4G res; 
    for (int i = 0; i < 4; ++i) { res[i] = Math::Floor(v1[i]); } 
    return res; 
} 
...
{ 
    Vector4G res; 
    for (int i = 0; i < 4; ++i) { res[i] = Math::Ceil(v1[i]); } 
    return res; 
} 
...
{ 
    Vector4G res; 
    for (int i = 0; i < 4; ++i) { res[i] = SCAST<T>( Math::Round(v1[i]) ); } 
    return res; 
} 
...
{ 
    Vector4G res = v; 
    for (int i = 0; i < 4; ++i) { res[i] = Math::Clamp(res[i], min[i], max[i]); } 
    return res; 
} 
...
{ 
    for (int i = 0; i < 4; ++i) { if (lhs[i] != rhs[i]) { return false; } } 
    return true; 
} 

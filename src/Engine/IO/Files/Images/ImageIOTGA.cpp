...
void *Bang::tgaMalloc(size_t size)
{
    return malloc(size);
}
void Bang::tgaFree(void *memory)
{
    free(memory);
}

int Bang::tgaGetWidth(const unsigned char *buffer)
{
    return (buffer[12] & 0xFF) | (buffer[13] & 0xFF) << 8;
}

int Bang::tgaGetHeight(const unsigned char *buffer)
{
...
static unsigned char *decodeRLE(int width,
                                int height,
                                int depth,
                                const unsigned char *buffer,
                                int offset);

static int *createPixelsFromColormap(int width,
                                     int height,
                                     int depth,
...
static int *createPixelsFromRGB(int width,
                                int height,
                                int depth,
                                const unsigned char *bytes,
                                int offset,
                                int descriptor,
                                const TGA_ORDER *order);
static int *createPixelsFromGrayscale(int width,
                                      int height,
                                      int depth,
                                      const unsigned char *bytes,
                                      int offset,
...
int *Bang::tgaRead(const unsigned char *buffer, const TGA_ORDER *order)
{
    // header
                    ...
                    for (i = 0; i < height; i++)
                    {
                        for (j = 0; j < width; j++)
                        {
                            int e = bytes[offset + 2 * width * i + 2 * j + 0] &
                                    0xFF;
                            int a = bytes[offset + 2 * width * i + 2 * j + 1] &
                                    0xFF;
                            pixels[width * i + (width - j - 1)] =
                                (e << rs) | (e << gs) | (e << bs) | (a << as);
                        }
